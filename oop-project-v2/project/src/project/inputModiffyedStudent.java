/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package project;

import java.awt.Color;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import javax.swing.*;

/**
 *
 * @author yousef
 */
public class inputModiffyedStudent extends JFrame implements ActionListener{
    JLabel Student_id;
    JButton Modiffy,Return;
    JTextField stdID;
   public inputModiffyedStudent()
    {
         this.setTitle("Modiffy Student");
    
    this.setSize(500, 400);
    this.setVisible(true);
    
    this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    
    this.setLayout(null);
    this.setLocation(450, 150);
    
    this.setResizable(true);
        
        
        Student_id=new JLabel("Student_ID");
        Modiffy=new JButton();
       stdID=new JTextField();
       Return=new JButton();
       Modiffy.setText("Modiffy");
       Return.setText("Return");
       Student_id.setBounds(80,75,200,40);
       stdID.setBounds(160,75,200,40);
       Modiffy.setBounds(160,145,200,40);
       Return.setBounds(160,215,200,40);
       
        Modiffy.setBackground(Color.GRAY);
        Modiffy.setForeground(Color.WHITE);
        
        Return.setBackground(Color.GREEN);
        Return.setForeground(Color.WHITE);
        
        
       this.add(Student_id);
       this.add(Modiffy);
       this.add(stdID);
       this.add(Return);
       Return.addActionListener(this);
       Modiffy.addActionListener(this);
       
       
    }
     @Override
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==Return)
        {
        AdminPage p=new AdminPage();
        this.dispose();
        }
        if(e.getSource()==Modiffy)
        {
            String std=stdID.getText();
            if(std.isEmpty())
                JOptionPane.showMessageDialog(this,"input is Empty", "Invalid Input",JOptionPane.ERROR_MESSAGE);
            else {
            boolean state=System_Operations.checkExist(std);
             if(state)
        {
        ModifyStudentPage nnn=new ModifyStudentPage();
        }
        else
             JOptionPane.showMessageDialog(this,"Student_ID not found", "ERROR",JOptionPane.ERROR_MESSAGE);
                    }
            
        }
    }
}
